...
export default function SalesPage() {
  ...
  // 🟢 AÑADIDO: Estados para métodos de pago
  const [paymentMethods, setPaymentMethods] = useState<{idPaymentMethod: number, namePaymentMethod: string}[]>([])
  ...
  // 1. Sincronizar catálogo y métodos de pago
  const loadData = () => {
    setLoading(true)
    Promise.all([
      fetch('http://127.0.0.1:8080/api/products').then(res => res.json()),
      fetch('http://127.0.0.1:8080/api/payment-methods').then(res => res.json())
    ])
    .then(([prodData, payData]) => {
      setProducts(prodData)
      setPaymentMethods(payData)
      if (payData.length > 0) setSelectedPaymentMethod(payData[0].idPaymentMethod)
      setLoading(false)
    })
    .catch((err) => {
            ...
            <div className="card border-0 shadow-sm order-summary">
              ...
              <div className="card-body">
                ...
                <div className="d-grid gap-2">
                  <button className="btn btn-success btn-lg" onClick={chargeCustomer} disabled={cart.length === 0}>
                    <i className="bi bi-credit-card me-2"></i>Confirm & Process
                  </button>
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header border-0">
                <h5 className="modal-title fw-bold"><i className="bi bi-file-earmark-check text-success me-2"></i>Invoice Generated</h5>
              ...
              <div className="modal-body">
                ...
                <table className="table mt-4">
                  <thead><tr><th>Product</th><th>Quantity</th><th className="text-end">Total</th></tr></thead>
                  <tbody>
              ...
              <div className="modal-footer border-0">
                <button className="btn btn-success" onClick={clearCart}>Complete Sale</button>
              </div>
            </div>
          </div>
        </div>
